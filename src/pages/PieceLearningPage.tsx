...
const PieceLearningPage: React.FC = () => {
    ...
    const [visitedPieces, setVisitedPieces] = useState<Set<string>>(new Set());
    ...
    const saveProgress = async () => {
        ...
        if (user) {
            ...
            Swal.fire({
                ...
                text: 'Has estudiado todas las piezas del ejército.',
                icon: 'success',
    ...
    const handleSquareSelect = (key: string) => {
        ...
        if (piece) {
            ...
            // Track visitation
            const newVisited = new Set(visitedPieces);
            newVisited.add(piece.role);
            setVisitedPieces(newVisited);

            if (newVisited.size === 6) { // 6 Unique pieces
                // Small delay to let user see the last piece
                if (visitedPieces.size === 5) { // Trigger only when reaching 6 from 5
