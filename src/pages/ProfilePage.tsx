            ...
            <div className="bg-white p-8 rounded-[2.5rem] shadow-cartoon-lg border-4 border-white max-w-md w-full relative z-10 mx-4">
                ...
                <form className="space-y-4">
                    <div className="space-y-2">
                        ...
                        <input type="email" placeholder="tu@email.com" className="w-full bg-background-light border-2 border-gray-200 rounded-2xl py-3 px-4 font-bold text-text-dark-fun focus:outline-none focus:border-primary-island focus:ring-4 focus:ring-primary-island/10 transition-all" />
                    </div>
                    <div className="space-y-2">
                        <label className="text-xs font-black uppercase text-gray-400 tracking-wider ml-2">Contraseña</label>
                        <input type="password" placeholder="••••••••" className="w-full bg-background-light border-2 border-gray-200 rounded-2xl py-3 px-4 font-bold text-text-dark-fun focus:outline-none focus:border-primary-island focus:ring-4 focus:ring-primary-island/10 transition-all" />
                    </div>

                    <button type="button" className="w-full bg-secondary-adventure hover:bg-yellow-400 text-primary-island font-black py-4 rounded-2xl shadow-[0_4px_0_rgb(180,130,0)] hover:shadow-[0_2px_0_rgb(180,130,0)] hover:translate-y-[2px] transition-all border-2 border-yellow-400 active:translate-y-[4px] active:shadow-none text-lg mt-4">
                        {isLogin ? 'Iniciar Sesión' : 'Crear Cuenta'}
                    </button>
                </form>

                <div className="mt-6 text-center">
                    <button onClick={() => setIsLogin(!isLogin)} className="text-sm font-bold text-gray-400 hover:text-primary-island transition-colors underline decoration-2 decoration-transparent hover:decoration-primary-island">
